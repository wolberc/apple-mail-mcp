...
def get_email(
    ...
    Returns:
        ...
        body (full text), and attachments list (name, size_bytes, mime_type).
    """
    # Normalise — callers may pass an integer
    ...
    tell application "Mail"
        with timeout of 60 seconds
            ...
            -- Use index-based loop to avoid AppleScript collection reference issues.
            -- Use "file size" (not "size") which is the correct Mail attachment property.
            set attachLines to {{}}
            ...
            repeat with i from 1 to realAttCount
                try
                    ...
                    set attSize to (file size of anAtt) as string
                    set attMime to ""
                    try
    ...
    # Annotate each attachment with its resource URI so clients can fetch the binary
...
def _parse_get_email_output(raw: str) -> Dict[str, Any]:
    ...
    for line in lines:
        ...
        # Collect continuation lines that belong to the ATTACHMENTS block
        # (each attachment is one linefeed-separated entry; they run until BODY_START)
        if in_attachments:
    ...
    # Parse attachment lines — each entry is "name|||size|||mime_type"
    attachments = []
    for att_line in attach_lines:
        ...
        if len(fields) >= 2:
            ...
            # AppleScript often returns empty mime type for Exchange/Outlook emails;
            # fall back to stdlib guess from the file extension.
    ...
    # Add mail_link from internet_message_id
...
# ---------------------------------------------------------------------------
# MCP Resource: attachment binary retrieval
# ---------------------------------------------------------------------------
...
def get_attachment_resource(message_id: Union[str, int], index: Union[str, int]) -> bytes:
    ...
    # Write to a temp file; AppleScript's `save` command requires a real path
